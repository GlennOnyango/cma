...
class Documents{
    ...
    public function getDocumentsTypes($data){
        ...
        $query = "SELECT `id`, `document_name`,`document_price`, `toc`, `document_description`, `subscriptions_name`,`category_name` FROM `vw_document` WHERE service_id=".$data['getDocumentsTypes'];
        $result = $this->db->query($query);
        $document = array();
        if (!$result) {
