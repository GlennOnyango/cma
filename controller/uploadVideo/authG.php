    ...
    public function getServiceDocuments($data){
        ...
        if (!$result) {
            ...
            while($row=$result -> fetch_assoc()){
                 ...
                 array_push($document,array("id"=>$row['id'],"Name"=>$row['document_name'],"price"=>$row['document_price'],"category_name"=>$row['category_name']));
...
class Videos{
    ...
    public function addvideo($data){
       ...
       if(!empty($_FILES['preview']) ){
        $preview = libs :: uploadFile($_FILES['videopreview'],"uploadVideoPreview"); 
    ...
    public function getServicevideos($data){
        ...
        $query = "SELECT `id`,`video_name`,`video`,`video_preview` FROM `vw_videos` WHERE service_id = ".$data;
        echo $query;
        $result = $this->db->query($query);
        //$video = array();
        if (!$result) {
